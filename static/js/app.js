...
App.booksController = Em.ArrayController.create({
  ...
  loadBooks: function() {
    var me = this;
    var username = '3679771';
    if ( username ) {
      var url = 'https://api.douban.com/v2/book/user/' + username + '/collections?count=100';
      $.ajax({
        url: url,
        context: this,
        dataType: 'jsonp',
        success: function(data){
          ...
          $(data).each(function(index,value){
            var t = App.Book.create({
                book_avatar: value.book.images.medium,
                book_author: value.book.author[0],
                book_title: value.book.title,
                book_price: value.book.price,
